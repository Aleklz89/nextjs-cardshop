...
function Page() {
  ...
  const calculateTotalCost = (depositAmount, cardsQty) => {
    ...
    const additionalPercentage =
      ((parseFloat(value) || 0) + (parseFloat(constant) || 0) + 5) / 100;
  ...
  const handleIssueCard = async () => {
    ...
    try {
      ...
      // After updating the balance, send the transaction record
      const transactionResponse = await fetch('/api/newtrans', {
        ...
        body: JSON.stringify({
          userId,
          type: 'withdrawal', // Assuming the transaction type you need
          description: 'Card Purchase',
          amount: -parseFloat(totalCost) // Send as a negative value
        }),
      });
