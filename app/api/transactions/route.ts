...
export async function POST(request: NextRequest) {
  try {
    ...
    const transactionIds = transactions
      .filter(transaction => transaction.type_enum === 'Authorization')
    ...
    for (const transaction of transactions) {
      if (transaction.type_enum === 'Authorization' && !existingHoldIds.has(transaction.id)) {
        const reverseTime = new Date();
        reverseTime.setHours(reverseTime.getHours() + 168);  // Устанавливаем 168 часов (7 дней)
